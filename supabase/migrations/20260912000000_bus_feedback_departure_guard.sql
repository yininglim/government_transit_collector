...
begin
  ...
  if statement_timestamp() < (
    (new.service_date::timestamp
      + new.scheduled_departure_seconds * interval '1 second')
    at time zone 'Asia/Kuala_Lumpur'
  ) then
    raise exception 'This trip has not started yet. You can report an issue after the scheduled departure time.' using errcode = '23514';
  end if;
  if not exists (
